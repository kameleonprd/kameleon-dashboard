 ...
 *
 * AWS Cognito authentication integration for Kameleon Dashboard.
...
// Kameleon user attributes from Cognito
export interface KameleonUser {
  userId: string;
  email: string;
  name?: string;
...
export async function signIn(
  ...
  try {
    ...
    // If user is already authenticated, treat as success
    if (
 ...
 */
export async function getUser(): Promise<AuthResult<KameleonUser>> {
  try {
    const user = await getCurrentUser();
    const attributes = await fetchUserAttributes();

    const kameleonUser: KameleonUser = {
      userId: user.userId,
      email: attributes.email || user.username,
      name: attributes.name,
      emailVerified: attributes.email_verified === "true",
      subscriptionTier: attributes["custom:subscription_tier"],
    };
...
export async function getSession(): Promise<AuthResult<SessionInfo>> {
  try {
    ...
    const expiresAt = session.tokens.accessToken?.payload?.exp
      ? new Date((session.tokens.accessToken.payload.exp as number) * 1000)
      : undefined;
